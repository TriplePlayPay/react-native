...
To edit the Objective-C files, open `example/ios/TPPSdkExample.xcworkspace` in XCode and find the source files at `Pods > Development Pods > tpp-react-native`.

To edit the Kotlin files, open `example/android` in Android studio and find the source files at `reactnativetppsdk` under `Android`.
...
- Run `yalc publish` in `@tripleplaypay/react-native` package to publish all the files that should be published in remote NPM registry.
- Run `yalc add @tripleplaypay/react-native` in your dependent project, which will copy the current version from the store to your project's .yalc folder and inject a file:.yalc/@tripleplaypay/react-native into package.json.
